...
import random
random_hr_values = [random.randint(0, 65535) for _ in range(1000)]
...
store_unint = ModbusSlaveContext(
  ...
  hr = ModbusSequentialDataBlock(0, random_hr_values)
)
context = ModbusServerContext(slaves={1: store_unint}, single=False)
StartTcpServer(context, address = ("192.168.27.30", 502))