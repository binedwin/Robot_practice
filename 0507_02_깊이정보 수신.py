...
#-----260507 추가_ 깊이 스트림 추가
config.enable_stream(rs.stream.depth, 640, 480, rs.format.z16, 30)
#----------------------------

profile = pipeline.start(config) #pipeline 시작

#깊이 센서 거리 계산을 하기위해 scale 정보 가져오겠습니다.
depth_sensor = profile.get_device().first_depth_sensor()
depth_scale = depth_sensor.get_depth_scale()
#깊이 정보를 가져오게 되는데, 일반적으로 metor 단위로 가져옵니다.

#정렬 (depth 정보를 color와 맞추기 위해선 정렬이 필요)
...
try:
  while True:
    ...
    #-------------중심점 좌표 표현 추가----------------------------------------
        # cv2.putText(color_image, f"{cx, cy}px", (cx-50, cy-20), cv2.FONT_HERSHEY_SIMPLEX, fontScale= 0.6, color= (255, 255, 255), thickness=2)
        cv2.putText(color_image, depth_data, (cx-50, cy-20), cv2.FONT_HERSHEY_SIMPLEX, fontScale= 0.6, color= (255, 255, 255), thickness=2)

    #------puttext 추가 실습
        cv2.putText(color_image, f"Vision OK", (320, 100), cv2.FONT_HERSHEY_SIMPLEX, fontScale= 0.6, color= (255, 255, 255), thickness=2)
    #------------------Pixel 수 표현
        cv2.putText(color_image, f"Pixel Area : {int(area)}", (320, 140), cv2.FONT_HERSHEY_SIMPLEX, fontScale= 0.6, color= (255, 255, 255), thickness=2)
    
    
    #if문 외부에 넣기
    cv2.putText(color_image, f"SSAFY 15th Vision", (40, 40), cv2.FONT_HERSHEY_SIMPLEX, fontScale= 1.2, color= (255, 255, 255), thickness=2)
